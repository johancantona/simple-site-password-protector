<head>
    ...
    <link rel="stylesheet" href="<?php echo plugins_url('simple-site-pwd-protector/dist/styles/sspp-form-frontend.css'); ?>" type="text/css" media="all" />
</head>
...
<?php
    ...
    if (isset($_POST['simple_site_pwd_protector_password'])) {
        ...
        if ($validation && $protector_id) {
            ...
            $url = strtok($url, '?');
            wp_redirect($url . '?pwd_failed=true');
        }
    }
    ...
    $failed_password_text = get_option('simple_site_pwd_protector_failed_password_text') ? get_option('simple_site_pwd_protector_failed_password_text') : 'Incorrect password';
    $error = isset($_GET['pwd_failed']) ? $failed_password_text : '';
?>
...
<div style="background-color:<?php echo $bg_color; ?>;width:100%;height:100vh;background-image:url(<?php echo $bg_image; ?>);background-size:cover;background-position:center center;background-repeat:no-repeat;display:flex;justify-content:center;align-items:center;flex-direction:column;">
    <form id="sspp-frontend-form" class="<?php echo $input_theme;?>" method="POST">
        <input type="password" name="simple_site_pwd_protector_password" placeholder="<?php echo $password_placeholder; ?>" />
